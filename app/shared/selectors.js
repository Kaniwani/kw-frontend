...
export const selectLevelEntities = createDeepEqualSelector(
  selectEntities,
...
export const selectUserLevel = createDeepEqualSelector(
  selectProfile,
  (state) => state && state.currentLevel
);
...
export const makeSelectLevelLoading = (id) =>
  createDeepEqualSelector(selectLevelsLoading, (loading) => loading.includes(id));
...
export const makeSelectQuizMeanings = (id) =>
  createDeepEqualSelector(
    ...
    (review) =>
      review
        ? filterRomajiReadings(review.vocabulary.meanings, review.vocabulary.readings)
        : []
  );
...
export const selectSessionCorrectIds = createDeepEqualSelector(
  selectSession,
  ({ correct }) => correct
);
export const selectSessionIncorrectIds = createDeepEqualSelector(
  selectSession,
  ({ incorrect }) => incorrect
);
...
export const makeSelectReviewsGroupedByRank = (ids) =>
  createDeepEqualSelector(
    ...
    (entities) =>
      ids.every(entityExists(entities))
        ? groupByRank(ids.map(addStreakToId(entities)))
        : {}
  );
