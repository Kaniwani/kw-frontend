...
export function getLevelEntry(body = {
  level: 0,
  offset: 0,
  limit: 100,
}) {
  const url = urls.levelEntry(body.id);
  return request.get({ url, body });
}

export function lockLevel(body = { id: null }) {
  const url = urls.lockLevel(body.id);
  return request.post({ url, body });
}

export function unlockLevel(body = { id: null }) {
  const url = urls.unlockLevel(body.id);
  return request.post({ url, body });
}
