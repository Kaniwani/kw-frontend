...
describe("combineFuri()", () => {
  ...
  it("accepts furi location string", () => {
    expect(combineFuri("お世辞", "おせじ", "1:せ;2:じ")).toEqual(
      expect.arrayContaining([["", "お"], ["せ", "世"], ["じ", "辞"]])
    );
  });
  it("renders appropriate fallbacks", () => {
    expect(combineFuri("すいか", "スイカ")).toEqual(
      expect.arrayContaining([["スイカ", "すいか"]])
    );
  });
  it("renders appropriate fallbacks", () => {
    expect(combineFuri("大人しい", "おとなしい")).toEqual(
      expect.arrayContaining([["おとな", "大人"], ["", "しい"]])
    );
  });
  it("renders appropriate fallbacks", () => {
    expect(combineFuri("使い方", "つかいかた")).toEqual(
      expect.arrayContaining([["つかいかた", "使い方"]])
    );
  });
  it("handles special readings for better display", () => {
    expect(combineFuri("今日", "きょう", "0:きょう")).toEqual(
      expect.arrayContaining([["きょう", "今日"]])
    );
  });
});
...
describe("parseFuriString()", () => {
  ...
  it("works", () => {
    expect(parseFuriString("1:せ;2:じ")).toEqual(
      expect.arrayContaining([[[1, 2], "せ"], [[2, 3], "じ"]])
    );
  });
});
