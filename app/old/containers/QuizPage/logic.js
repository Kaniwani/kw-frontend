...
import { isEmpty, sample, difference } from 'lodash';

import { recordReview } from 'common/api';
import { SRS_RANGES } from 'common/constants';
import fixTerminalN from 'common/utils/fixTerminalN';
import matchAnswer from 'common/utils/matchAnswer';
import combineAnswers from 'common/utils/combineAnswers';

import increment from 'common/utils/increment';
import decrement from 'common/utils/decrement';
...
export const recordAnswerLogic = createLogic({
  ...
  process({ action }, dispatch, done) {
    const { category, current, isCorrect, previouslyIncorrect } = action.payload;
    clearTimeout(autoAdvanceTimeout);
