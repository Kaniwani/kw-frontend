...
export const getBy = (val, transform = identity) => (state) => compose(
  transform,
  partialRight(get, val)
)(state);
...
export const makeSelectItemIds = (domainSelector) => createSelector(domainSelector, (domain) => Object.keys(domain).map(Number));

export const makeSelectItemById = (itemsSelector) => createSelector([itemsSelector, getProp('id')], (items, id) => items[`${id}`] || {});
