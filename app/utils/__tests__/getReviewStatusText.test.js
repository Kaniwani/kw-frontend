...
describe("getReviewStatusText()", () => {
  ...
  it("no reviews unlocked", () => {
    expect(
      getReviewStatusText({ reviewsCount: 0, nextReviewDate: false })
    ).toMatchSnapshot();
  });
  it("review date is past", () => {
    expect(
      getReviewStatusText({ reviewsCount: 0, nextReviewDate: subHours(new Date(), 2) })
    ).toMatchSnapshot();
  });
  it("review date is future", () => {
    expect(
      getReviewStatusText({ reviewsCount: 0, nextReviewDate: addHours(new Date(), 2) })
    ).toMatchSnapshot();
  });
});
